...
template <class T> struct SNode
{
	...
	bool is_infinity()
	{
		return std::holds_alternative<Infinity>(
			value); // проверяет что std::variant хранит infinity
	}
...
bool operator<(const std::variant<Infinity, T> &op1, const std::variant<Infinity, T> &op2)
{
	...
	{
		if (std::get<Infinity>(op1) == Infinity::MINUS_INFINITY &&
			std::get<Infinity>(op2) == Infinity::PLUS_INFINITY)
			result = true;
		else
			result = false;
	}
	...
	{
		if (std::get<Infinity>(op2) == Infinity::PLUS_INFINITY)
			result = true;
		else
			result = false;
	}
	...
	{
		if (std::get<Infinity>(op1) == Infinity::PLUS_INFINITY)
			result = false;
		else
			result = true;
	}
	else
		result = (std::get<T>(op1) < std::get<T>(op2));
	return result;
}
