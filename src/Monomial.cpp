...
{
	return (coefficient * pow(x_value, (*this)[0]) * pow(y_value, (*this)[1]) *
			pow(z_value, (*this)[2]));
}
...
Monomial::Monomial(double coefficient, signed char x_degree, signed char y_degree,
...
{
	if (place < 0 || place > 3)
		throw std::out_of_range("invalid place. Function: Monomial::change_degrees\n");
...
{
	return Monomial(m.coefficient * coefficient, m.degrees.degree[0], m.degrees.degree[1],
					m.degrees.degree[2]);
}
...
{
	ostr << "[" << m.coefficient << "*" << "x^" << (int)m[0] << "*" << "y^" << (int)m[1] << "*"
		 << "z^" << (int)m[2] << "]";
	return ostr;
}